...
import com.pathshala.schedule.syllabus.fileuploaddownload.repository.FileDataRepository2;
...
public class FileDataServiceImpl2 implements FileDataService2 {
	
	private final FileDataRepository2 fileDataRepository;

	@Value("${upload-dir}")
	private String FOLDER_PATH;

	private Path root = Paths.get("C:/Pathshala/syllabusNTimetable/files2/");
	...
	public ResponseData uploadFile(MultipartFile file, Integer classId, Integer sectionId, Integer sessionId)
		...
		try {
			...
			if (fileName.startsWith(Constants.TIMETABLE_FILE_TYPE)) {
				fileData = new FileData2(classId, sectionId, sessionId, fileName, file.getContentType(), filePath, Constants.TIMETABLE_FILE_TYPE);
			} else if (fileName.startsWith(Constants.SYLLABUS_FILE_TYPE)) {
				fileData = new FileData2(classId, sectionId, sessionId, fileName, file.getContentType(), filePath, Constants.SYLLABUS_FILE_TYPE);
			} else {
				fileData = new FileData2(classId, sectionId, sessionId, fileName, file.getContentType(), filePath, Constants.OTHERS_FILE_TYPE);
			}
			...
			FileData2 fileDate = fileDataRepository.save(fileData);
	...
	public ResponseData updateFile(MultipartFile file, Integer classId, Integer sectionId, Integer sessionId,
		...
		try {
			...
			if (fileName.startsWith(Constants.TIMETABLE_FILE_TYPE)) {
				fileData = new FileData2(classId, sectionId, sessionId, fileName,
...
//					filename);
	...
	public ResponseData load(String filename, Integer classId, Integer sectionId, Integer sessionId) {
		try {
			...
			FileData2 fileData = fileDataRepository
					.findByClassIdAndFileName(new ClassId(classId, sectionId, sessionId), filename);
	...
	public boolean deleteByFileName(String filename, Integer classId, Integer sectionId, Integer sessionId) {
		try {
			// delete from d/b
			fileDataRepository.deleteByClassIdAndFileName(new ClassId(classId, sectionId, sessionId),
					filename);
